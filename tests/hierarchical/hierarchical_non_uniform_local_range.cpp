...
void check_dim(util::logger &log) {
  ...
  const unsigned group_range_2d = (dim > 1) ? ((dim > 2) ? group_range :
      group_range_total / group_range) : 1;
  const unsigned group_range_3d = (dim > 2) ? group_range : 1;
  const unsigned local_range_1d = (dim > 1) ? local_range : local_range_total;
  const unsigned local_range_2d = (dim > 1) ? ((dim > 2) ? local_range :
      local_range_total / local_range) : 1;
      ...
      myQueue.submit([&](cl::sycl::handler &cgh) {
        ...
        auto groupRange =
            sycl_cts::util::get_cts_object::range<dim>::template
              get_fixed_size<group_range_total>(group_range, group_range);
        auto localRange =
            sycl_cts::util::get_cts_object::range<dim>::template
              get_fixed_size<local_range_total>(local_range, local_range);
        cgh.parallel_for_work_group<kernel<dim>>(
            groupRange, localRange,
            [=](cl::sycl::group<dim> group_pid) {
              group_pid.parallel_for_work_item(
                  ...
                  [&](cl::sycl::h_item<dim> item_id) {
                    ...
                    unsigned physical_local_d2 = (dim > 1) ?
                        item_id.get_physical_local()[1] : 0;
                    unsigned physical_local_d3 = (dim > 2) ?
                        item_id.get_physical_local()[2] : 0;
                    ...
                    unsigned globalIdL = ((globalId1 * global_range2 *
                                            global_range3) +
                                          (globalId2 * global_range3) +
                                          globalId3);
                    // Assign local item work-item’s position in the local range
                    // but in new non-uniform logical local range that depends on a work-group id
                    accessor[globalIdL] =
